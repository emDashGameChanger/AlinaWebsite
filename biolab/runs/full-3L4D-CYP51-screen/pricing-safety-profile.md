...
|---|---|---|---|
| Digitoxin | Heart failure, arrhythmia (historically) | **Not marketed in the US** (FDA: no longer manufactured there) or Canada; in Europe, currently only Germany (in shortage), see availability notes below; digoxin (its still-marketed cousin) runs ~$24+/month generic | Narrow therapeutic index, cardiac glycoside toxicity; N/A for current US prescribing |
...
- Availability outside the US (researched 2026-09-18, independently verified by Gemini the same day; full log in `~/Projects/Alina/digitoxin-availability.md`, outside this repo):
  - US: [IARC monograph, NCBI Bookshelf](https://www.ncbi.nlm.nih.gov/books/NBK350386/) (Vol. 108, 2016, citing Index Nominum 2013): FDA reported digitoxin no longer manufactured in the USA; "only seven licensed products containing digitoxin in Germany, Austria, Hungary, and Norway". The FDA Orange Book lists Crystodigin and Acylanid as discontinued (`DISCN`).
  - Germany: [apotheke adhoc](https://www.apotheke-adhoc.de/nachrichten/detail/pharmazie/verpackung-eingestellt-digitoxin-fehlt-bis-2027/): Digitoxin AWD 0.07 (Teva) is the only oral product left, the only country still marketing it; supply shortage 1 July 2026 to expected 29 January 2027.
  - Austria, Hungary, Norway: the 2013 IARC list is out of date. Per the log (Gemini-verified), Austria's Digimerck registrations are "aufgehoben" (cancelled), Hungary's Digimerck 0.1 mg (OGYI-T-04084) is "TT" (cancelled) since 26 July 2023, and Norway's Digitoksin Nycomed is under "utgåtte legemidler" (discontinued). Not re-checked by Claude Code.
  ...
  - Visceral-leishmaniasis-endemic countries: Brazil, ANVISA open data (`DADOS_ABERTOS_MEDICAMENTOS.csv`): zero active registrations, two historical ones both `Inativo` (expired 2002 and 2006). India, CDSCO/SUGAM: 0 results for digitoxin (digoxin returns many). Bangladesh, MedEx: 0 digitoxin products. These three are record-level checks. Kenya (PPB) is search-snippet only, and Sudan, Ethiopia, Nepal, South Sudan and Somalia are web-search "not found" only, so there it is "not found," not "confirmed absent." Uganda and Yemen not searched.
...
elsewhere it is barely marketed at all: only Germany, currently in shortage
(see availability notes above). Being off the
